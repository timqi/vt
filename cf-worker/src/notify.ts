...
// not client-claimed meta) joins the head line as `user@host · N records`, mirroring the
...
export function metaLines(
  ...
  // The parent-process line mirrors the approval page's Parent process row: users often
  ...
  if (meta.ip) lines.push(meta.ip_prev ? `ip: ${meta.ip} (last ${meta.ip_prev})` : `ip: ${meta.ip}`);
  if (meta.reason) lines.push(`reason: ${meta.reason}`);
  return lines;
}

// `user@host · N records · note`, then pwd and the command — the block both notices
...
function bodyLines(
  ...
  const head = [who, salts > 0 ? `${salts} records` : '', ...extra].filter(Boolean).join(' · ');
...
// (no Touch ID). `salts` of 0 (e.g. an agent `sign` hit has no records) drops
// the count segment rather than printing "0 records". `names` are the served
// records' labels (owned name, client claim or salt prefix — account_names.nameLabel),
