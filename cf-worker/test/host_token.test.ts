...
describe('host token derivation', () => {
  it('pins the HKDF golden vector shared with the Rust client', async () => {
    const secret = await deriveHostTokenSecret(new TextEncoder().encode('test-master-token'), 'AAAAAAAAAAAAAAAA');
  ...
  it('formats vt1.<id>.<secret> and derives a different secret per id', async () => {
    const root = new TextEncoder().encode('test-master-token');
    const tok = formatHostToken('AAAAAAAAAAAAAAAA', await deriveHostTokenSecret(root, 'AAAAAAAAAAAAAAAA'));
    expect(tok).toBe('vt1.AAAAAAAAAAAAAAAA.iaR45SwFl4C19e0hLGVnh32aBZlyjE4i47Jp_FbuKAI');
    const other = formatHostToken('BBBBBBBBBBBBBBBB', await deriveHostTokenSecret(root, 'BBBBBBBBBBBBBBBB'));
