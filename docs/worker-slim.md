...
| --- | --- | --- |
| Pushover, Slack App, Feishu | `pushover.ts`, `slack_app.ts`, `feishu.ts`; `fanOut`/`notifyApproval`/`notifyCacheHit` in `notify.ts`; `EditableChannel`, `feishuKv`, `sendAndStore` in `account_notifications.ts`; `Challenge.feishu_message_id`, `Challenge.slackapp`, `SlackAppMsgRef`; `push_warning` on `ChallengeResponse`/`EnrollResponse` and in `src/cf.rs`; `docs/slack-app.md`, `docs/feishu.md`, `docs/feishu-design.md` | `wrangler secret delete PUSHOVER_JSON SLACK_APP_JSON FEISHU_JSON`; subscribe the phone on the 设置 tab |
| Channels admin page | `pwa/admin/channels.html`, `channels.js`, `channelVars` in `page.ts`, `/<seg>/channels` route | none |
...
| Per-tab admin shells | `pwa/admin/{audit,cache,tokens,setup}.html`, `adminTabs` | none |
...
`approve.js` ceremony, `libsodium.js`. refactor.md §4 "Notification channels"
is decided here; that bullet goes when §7 step 2 lands.
...
where `pwa/` changed. Steps 1–3 are independent of refactor.md step 3 (cache
key v5); steps 4–5 land after it.

| # | Change | Files | Tests moving to rejected-input |
| --- | --- | --- | --- |
| 1 | **Web Push added** beside the existing channels | new `webpush.ts`, `account_admin.ts` (config blob under `K_cfg` — `vapid` + `push` only for now, key derived from `env.VT_AUTH_CF`), `pwa/sw.js`, `pwa/manifest.webmanifest`; `account_notifications.ts` (push fan-out, 410 drop), `index.ts` (`/sw.js`, `/manifest.webmanifest`, `/<seg>/api/push/*`), a 推送 tab; `types.ts`; `dek-cache.md` cache-hit paragraph | none; new `webpush.test.ts` (RFC 8291 vector, VAPID header, 3993 cap), `account_admin.test.ts` (upsert, cap 10, 410 → drop, 5xx → keep) |
| 2 | **Channels deleted** | rows 1–2 of §1; `src/cf.rs` `push_warning`; `wrangler.toml.example`; `cf-worker-deploy.md`; AGENTS.md notification line; refactor.md §4 bullet | `do_account.notifications.test.ts`: Feishu/Slack cases deleted, cache-hit opt-in/throttle re-targets push, "returns push_warning" → asserts the key is absent; `page.test.ts` channels shell case deleted |
| 3 | **One admin shell**, assets public, ui-ux doc | `pwa/admin/admin.html` + `admin.js` replace four shells and `adminTabs`; `isAdminAssetPath` and the `/<seg>/pwa/*` mount go; `docs/design/ui-ux.md`; `docs/README.md`; AGENTS.md asset line (Q1) | `page.test.ts` `isAdminAssetPath` cases → "public `/pwa/admin/admin.js` serves 200"; `adminTabs follows ADMIN_SEG` deleted |
