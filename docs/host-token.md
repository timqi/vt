...
4. When `auth.legacy_master` stops appearing, delete the legacy branch in
   `readAuthenticatedDaemonBody` and the hostname-keyed audit derivation, and
   rotate `VT_AUTH_CF` once (which also rotates every host token secret — hosts
   must re-enroll; do it while they are reachable).
