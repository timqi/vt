...
CLI (vt read / write / auth)
  |
  +-- try $SSH_AUTH_SOCK / ~/.ssh/vt.sock  (macOS: vt ssh agent)
  |    -> SSH extension protocol encrypt@vt / decrypt@vt / auth@vt
  |
  +-- CF ceremony fallback (Linux + macOS without agent):
       1. Load ~/.config/vt/cf_config.json
...
`~/.config/vt/cf_config.json` (mode 0600):
```json
{
  "worker_url": "https://vt-passkey.example.com",
  "worker_auth": "<base64url token>"
}
```

Same `worker_auth` must be set as `VT_AUTH_CF` wrangler secret.
