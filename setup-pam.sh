...
#   1. SSH agent (macOS Touch ID) over a forwarded agent — needs only the
#      forwarded socket (recovered from the invoking shell at sudo time).
#   2. Phone passkey (Cloudflare Worker) — needs VT_PASSKEY_URL + VT_PASSKEY_TOKEN.
# A host may use either or both. On a plain Linux server with no forwarded
# macOS agent, only the worker path applies.
#
...
# and the `=`/whitespace boundary prevents prefix collisions (VT_PASSKEY_URL vs
# VT_PASSKEY_UV).
read_cfg() {
    [ -r "$CFG" ] || return 0
    awk -v key="$1" '
...
# Report which path(s) will be wired in. The agent path is always wired: it
# engages whenever the sudo session carries a forwarded vt agent socket.
paths="SSH-agent (Touch ID, when forwarded)"
[ -n "$VT_PASSKEY_URL" ] && paths="$paths + phone passkey (worker)"
[ -z "$VT_PASSKEY_URL" ] && echo "  note: VT_PASSKEY_URL unset — without a forwarded agent, sudo falls through to the password stack" >&2
...
# Agent path (macOS Touch ID over a forwarded SSH agent). pam_exec doesn't
# inherit the user's env, so recover SSH_AUTH_SOCK from the invoking shell via
# /proc: user shell -> sudo -> pam_exec -> here.
...
if [ -z "${SSH_AUTH_SOCK:-}" ] && [ -z "$VT_PASSKEY_URL" ]; then exit 1; fi

# vt routes agent-first (if the socket is reachable), then falls back to
...
echo "  Tip: subscribe your phone on the admin Settings → Push tab so approvals reach it directly."
